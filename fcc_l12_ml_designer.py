...
# ============================================================
#  1. Element Database & Enthalpy Matrix (Same as before)
# ============================================================
ELEMENTS_DATA = {
...
# ============================================================
#  3. ML Criteria for FCC + L12 (Based on provided paper)
# ============================================================
def check_FCC_L12(VEC, dH, Tm):
    """
    Based on the Machine Learning rule extracted from the paper for FCC + L12 dual-phase:
    1. VEC > 8
    2. -16.0 < ΔH_mix < -9.7 kJ/mol
    3. 1671 < Tm < 1822 K
    """
    results = {}
    ...
    # 1. VEC
    if VEC > 8:
        results["ML Rule 1: VEC > 8"] = "✅ Pass"
    else:
        results["ML Rule 1: VEC > 8"] = f"❌ Fail ({VEC:.2f})"
        
    # 2. Enthalpy (dH)
    if -16.0 < dH < -9.7:
        results["ML Rule 2: -16.0 < ΔH_mix < -9.7"] = "✅ Pass"
    else:
        results["ML Rule 2: -16.0 < ΔH_mix < -9.7"] = f"❌ Fail ({dH:.2f})"
        
    # 3. Melting Temp (Tm)
    if 1671 < Tm < 1822:
        results["ML Rule 3: 1671 < T_m < 1822"] = "✅ Pass"
    else:
        results["ML Rule 3: 1671 < T_m < 1822"] = f"❌ Fail ({Tm:.0f} K)"
...
def show_criteria(title, criteria):
    ...
    for key, val in criteria.items():
        ...
        print(f"{marker}  {key:<38} {val}")

def calculate_all_params(elements, fracs):
    ...
    ml_results = check_FCC_L12(VEC, dH, Tm)
    passed = all("✅" in v for v in ml_results.values())
...
def main():
    print("=" * 68)
    print("   ML-BASED FCC + L12 DUAL-PHASE ALLOY DESIGNER")
    print("=" * 68)
    
    raw = input("\n  Enter elements separated by space (e.g. Ni Co Cr Al Ti): ")
    ...
    print(f"\n  ⚙️ Scanning combinations based on ML Rules... (Step={step*100:.0f}%)")
    
    for fracs in generate_compositions(step, len(elements)):
        data = calculate_all_params(elements, fracs)
        if data["is_valid"]:
            valid_data.append(data)
            
    if not valid_data:
        print("\n  ❌ No combinations met the ML criteria for FCC + L12.")
        print("  💡 Tip: L12 phase typically requires elements like Al, Ti forming Ni3(Al,Ti).")
        return

    print(f"  ✅ Found {len(valid_data)} highly accurate FCC + L12 combinations.")
    ...
    params_to_show = ["dH", "dr", "VEC", "Tm", "E_FCC"]
    labels = {
        "dH": "ΔH_mix (kJ/mol)", "dr": "δ_r (%)", "VEC": "VEC (-)", 
        "Tm": "Tm (K)", "E_FCC": "DFT E_FCC (eV)"
    }
    ...
    # ── 3. The "Best" Example ──
    # برای فاز L12+FCC علاوه بر پایداری، نزدیک بودن به مرکز رنج دمایی و آنتالپی هم مهم است، 
    # در اینجا ترکیب با پایین‌ترین E_FCC را نشان می‌دهیم.
    best_data = sorted(valid_data, key=lambda x: x["E_FCC"])[0]  
    
    print(f"\n{'='*68}\n  EXAMPLE: MOST STABLE FCC+L12 CANDIDATE (Lowest E_FCC)\n{'='*68}")
    ...
    show_criteria("ML Derived Rules (FCC + L12 Phase)", best_data["ml_results"])
    print("=" * 68)
